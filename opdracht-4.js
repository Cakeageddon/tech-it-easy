...
} console.log(tvNameString(inventories[1]))
...
function tvPriceString(tv) {
    return `€${tv.price},-.`
} console.log(tvPriceString(inventories[1]))
...
//     Als een tv maar één schermgrootte heeft ([32]) wordt de output 32 inch (81 cm). Wanneer een tv vier schermgroottes heeft ([43, 50, 55, 58]) wordt de output 43 inch (109 cm) | 50 inch (127 cm) | 58 inch (147 cm). Test of jouw functie werkt in alle mogelijke gevallen.

function tvSizeString(sizesArr) {
    let availabletvsizes = ""
    for (let i = 0; i < sizesArr.length; i++) {
        const currentSizeInches = sizesArr[i]
        const currentSizeCm = sizesArr[i] * 2.54
        availabletvsizes = availabletvsizes + `${currentSizeInches} inch (${currentSizeCm} cm)`
        if (i < sizesArr.length - 1) {
            availabletvsizes = `${availabletvsizes} | `
        }
    } return availabletvsizes
} console.log(tvSizeString(inventories[0].availableSizes))
...
tvName.innerHTML = tvNameString(inventories[0])
const tvPrice = document.getElementById("tv-price")
tvPrice.innerHTML = tvPriceString(inventories[0])
const tvSize = document.getElementById("tv-size")
tvSize.innerHTML = tvSizeString(inventories[0].availableSizes)
...
function allTheTvs(tvInventory) {
    const tvInventoryBundle = document.getElementById("tv-inventory")
    const tvModels = tvInventory.map((tvModels) => {
        return `
        <li>
            <h3>${tvNameString(tvModels)}</h3> 
            <p>${tvPriceString(tvModels)}</p>
            <p>${tvSizeString(tvModels.availableSizes)}</p>
        </li> `
    });
    tvInventoryBundle.innerHTML = `${tvModels}`;
}
allTheTvs(inventories)